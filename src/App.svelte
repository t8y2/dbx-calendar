<script>
  ...
  const copy = {
    en: {
      ...
      noStorage: "This host blocks local storage, so notes last only until it closes. Export to keep them.",
    ...
    zh: {
      ...
      noStorage: "当前宿主禁用了本地存储，备忘在关闭后会丢失，请先导出保存。",
  ...
  /** The host's storage handle, or null when the host does not offer one. */
  let storage = null;
  ...
  /** Writes the whole note map back to the host. Fire-and-forget so callers stay
   *  synchronous; a rejected write surfaces as a toast rather than vanishing. */
  function persist() {
    storage?.set(STORAGE_KEY, notes).catch((error) => flash(`${text.saveFailed}: ${error.message}`));
  }
  ...
  /**
   * Notes cannot go in localStorage: the plugin frame runs on an opaque origin
   * and every access throws. The host's own storage API is the supported route,
   * but it is gated on a permission, so both the advertised capability and the
   * handle are checked before either is used.
   */
  async function init() {
    const host = window.dbxPlugin;
    if (!host) {
      // Opened directly in a browser: nothing to persist to.
      storageOk = false;
      return;
    }
    ...
    const context = await host.ready?.catch(() => null);
    const locale = context?.locale ?? host.locale;
    if (locale?.toLowerCase().startsWith("zh")) lang = "zh";

    const advertised = context?.capabilities?.storage ?? host.capabilities?.storage;
    storage = advertised && host.storage ? host.storage : null;
    storageOk = Boolean(storage);
    console.info(`[calendar] host storage: ${storageOk ? "available" : "unavailable"}`);
    if (!storage) return;

    try {
      notes = (await storage.get(STORAGE_KEY)) ?? {};
    } catch (error) {
      console.warn("[calendar] could not read stored notes:", error);
      notes = {};
    }
  }
...
<main>
  ...
  {#if !storageOk}
    <p class="notice">{text.noStorage}</p>
  {/if}
